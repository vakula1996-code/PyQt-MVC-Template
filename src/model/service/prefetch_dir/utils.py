...
from .prefetch_read import DecompressWin10
...
class Prefetch(object):
	...
	def parseHeader(self, infile):
		...
		self.executableName = \
		struct.unpack_from("60s", infile.read(60))[0].decode("UTF-16", errors="backslashreplace").split("\x00")[0]
	...
	def getTimeStamps(self, lastRunTime):
		...
		for timestamp in range(8):
			try:
				...
				if ts:
					self.timestamps.append(self.convertTimestamp(ts))
			except struct.error:
	...
	def directoryStrings(self, infile):
		...
		while count < self.dirStringsCount:
			...
			directoryStrings.append(
				(infile.read(stringLength).decode("UTF-16", errors="backslashreplace")).split("\x00")[:-1])
			count += 1
	...
	def prettyPrint(self):
		...
		if len(self.timestamps) > 0:
			...
			for timestamp in self.timestamps:
				last_executed.append(timestamp)

		else:
			last_executed.append('Час відсутній')
		...
		for volume in self.directoryStringsArray:
			for dirstring in enumerate(volume):
				# print("{:>4}: {}".format(dirstring[0], ))
				directory_strings.append(dirstring[1][0])
			# d =dirstring[1]
			# my_list.append({d})
			# print(my_list)
		# print()
		...
		for resource in enumerate(self.resources):
			...
			resources_loaded.append(resource[1])
		# print()
		# my_list1.append({'res' : "{:>4}: {}".format(resource[0], resource[1])})
		# print(my_list1)
		# my_list.append({'exept' : "{}".format(self.executableName)})
		# 'directory' : "{:>4}: {}".format(dirstring[0], dirstring[1]),
		# 'resourse' : "{:>4}: {}".format(resource[0], resource[1]),
		# 'execute' : "{}".format(self.timestamps[0])
		return {
			'name_file': name_file,
